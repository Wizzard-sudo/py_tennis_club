...
class MembersModelTests(TestCase):
    ...
    def test_should_return_list_of_members(self):
        member1 = create_member('John', 'Dou', 'john.dou@test.com', '1234567')
        member2 = create_member('Max', 'Pain', 'max.pain@test.com', '7654321')

        response = self.client.get(reverse("members:members"))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, str(member1) + str(member2))
